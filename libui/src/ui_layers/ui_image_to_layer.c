...
/*   Updated: 2019/07/26 00:51:19 by alerandy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ui_layer		*ui_image_to_layer(char *path)
{
	...
	if (!(layer = ft_memalloc(sizeof(t_ui_layer))))
		return (NULL);
	bmp = ui_getbmp(path);
	if (!bmp.pixels || !(layer->pixels = ft_memalloc(sizeof(unsigned) \
														* bmp.pixel_count)))
	{
		ft_putendl_fd(!bmp.pixels ? "BMP Failed." : "Layer allocation failed.", 2);
